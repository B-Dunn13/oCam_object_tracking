...
w_cx = ? #Angular velovity of the drone
w_cy = ? #Angular velovity of the drone
w_cz = ? #Angular velovity of the drone
v_tx = 0 #Target velocity
v_ty = 0 #Target velocity
v_tz = 0 #Target velocity
...
InteractionMatrix = np.matrix([[(-1 / D), 0, (x / D), (x * y), -(1 + math.pow(x,2)), y],
	[0, (-1 / D), (y / D), (1 + math.pow(y,2)), (-x * y), -x]]) # D is the depth of the point relative to the camera frame
...
PseudoInverse = inv(L_e.transpose * L_e) * L_e.transpose # Moore-Penrose pseudo inverse

CameraVelocity = np.matrix([[v_cx], [v_cy], [v_cz], [w_cx], [w_cy], [w_cz]]) # input to the controller

# Equation 3.8: camera velocity relative to target velocity
v_c = np.matrix([[v_cx - v_tx], [v_cy - v_ty], [v_cz - v_tz], [w_cx], [w_cy], [w_cz]])
